...
from sdas.api.routes.utils import get_session

from .type import SignalType

sdas_events = Namespace()
...
class SignalFactory:
    ...
    pre_acquisition: Signal = sdas_events.signal('pre-acquisition')
    pre_company: Signal = sdas_events.signal('pre-company')
    pre_feature: Signal = sdas_events.signal('pre-feature')
    post_feature: Signal = sdas_events.signal('post-feature')
    post_company: Signal = sdas_events.signal('post-company')
    post_acquisition: Signal = sdas_events.signal('post-acquisition')

    on_error: Signal = sdas_events.signal('on-error')
    ...
    def get_signals_for_type(cls, signal_type: SignalType):
        ...
        Parameters
        ----------
        :class:`sdas.api.signals.SignalType`:
